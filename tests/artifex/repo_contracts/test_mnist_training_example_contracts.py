...
STREAMING_PIPELINE_NOTE = "for batch in train_pipeline"
BANNED_STREAMING_PIPELINE_REFERENCES = [
    "Pipeline.step is @nnx.jit by default",
    "pipeline.step(",
]
...
def test_mnist_training_examples_use_live_datarax_tfds_helper() -> None:
    ...
    for path in paths:
        ...
        assert "eager=False" in contents, f"{path} should force the documented streaming path"
        assert STREAMING_PIPELINE_NOTE in contents, f"{path} should document Pipeline iteration"
        ...
        for banned_reference in BANNED_STREAMING_PIPELINE_REFERENCES:
            assert banned_reference not in contents, f"{path} still references {banned_reference!r}"
