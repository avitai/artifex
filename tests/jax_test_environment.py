"""The JAX environment an artifex test run starts with: backend choice and CPU device emulation.

``substrax.runtime.resolve_test_runtime`` decides it; this module holds artifex's part, the
``ARTIFEX_TEST_`` variable prefix and how an installed JAX CUDA plugin is detected. Tests run on
CPU with eight emulated devices unless ``ARTIFEX_TEST_JAX_PLATFORMS`` asks for an accelerator, and
...
TEST_VARIABLE_PREFIX = "ARTIFEX_TEST_"
