...
usage() {
    cat <<'EOF'
Artifex development environment setup
...
Options:
  ...
  --force-clean                      Remove .venv, the generated .artifex.env, and repo-local test artifacts
  --dry-run                          Print the resolved backend and uv commands without changing files
  --help, -h                         Show this help

Notes:
  - Artifex uses uv for all repo-maintained setup and test workflows.
  ...
  - The generated backend file is .artifex.env. User-owned .env is not modified.
EOF
}
