...
def pytest_runtest_setup(item):
    """Set up for each test run - verify GPU availability for GPU-marked tests."""
    try:
        import jax

        # Only check GPU availability for GPU-marked tests
...
def pytest_runtest_teardown(item, nextitem):  # noqa: ARG001
    """Teardown after each test run - clean up GPU memory for GPU tests only."""
    try:
        # Only run cleanup for GPU-marked tests to avoid overhead on every test
        if hasattr(item, "get_closest_marker"):
            gpu_marker = item.get_closest_marker("gpu")
            cuda_marker = item.get_closest_marker("cuda")
