#!/usr/bin/env python
"""Unified environment setup utility for Artifex.
...
def run_command(
    cmd: str, desc: str | None = None, exit_on_error: bool = True, print_output: bool = True
...
def create_env_file(has_cuda: bool = False) -> None:
    ...
    if has_cuda:
        ...
        if template_path.exists():
            ...
            with open(template_path, "r") as src, open(".env", "w") as dst:
                dst.write(src.read())
            log_success("Created GPU-enabled .env from template")
        else:
            # Create GPU configuration matching setup.sh
            env_content = f"""# Artifex Environment Configuration - GPU Enabled
...
"""
        ...
        env_content = f"""# Artifex Environment Configuration - CPU Only
# Auto-generated by setup_env.py
...
def print_environment_status() -> None:
    ...
    print(f"{Colors.BLUE}🔍 Artifex Environment Status{Colors.NC}")
    print("=" * 40)
...
def clean_environment(deep_clean: bool = False) -> None:
    ...
    # Basic cleaning
    if Path(".venv").exists():
        run_command("rm -rf .venv", desc="Removing virtual environment", exit_on_error=False)
    ...
    if deep_clean:
        ...
        if jax_cache.exists():
            run_command(f"rm -rf {jax_cache}", desc="Removing JAX cache", exit_on_error=False)

        # pytest cache
        if Path(".pytest_cache").exists():
            run_command("rm -rf .pytest_cache", desc="Removing pytest cache", exit_on_error=False)

        # Coverage files
        run_command(
            "rm -f .coverage .coverage.*", desc="Removing coverage files", exit_on_error=False
        )
        if Path("htmlcov").exists():
            run_command("rm -rf htmlcov", desc="Removing coverage HTML", exit_on_error=False)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Artifex environment setup and management utility",
...
""",
    ...
    # Create-env command
    env_parser = subparsers.add_parser("create-env", help="Create .env configuration file")
    env_parser.add_argument("--cpu-only", action="store_true", help="Create CPU-only configuration")
