#!/usr/bin/env python3
"""
Script to analyze dependencies in the artifex package.

This script uses pydeps to analyze dependencies in the artifex package,
detect circular dependencies, and generate report files.
"""
...
class DependencyAnalyzer:
    ...
    def generate_graph(
        ...
        # Create graph
        dot = graphviz.Digraph(comment="Module Dependencies", format=format, engine="dot")
...
def generate_dependency_report(
    dependencies: list[ModuleDependency], circular_dependencies: list[list[ModuleDependency]]
) -> str:
    """
    ...
    # Generate report
    report = ["# Module Dependency Analysis\n"]
    ...
    sorted_modules = sorted(set(list(dependencies_count.keys()) + list(dependants_count.keys())))
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Analyze dependencies in the artifex package.")
    parser.add_argument(
        "--output-dir",
        default="docs/images",
    ...
    parser.add_argument(
        ...
        help="Path to save the dependency report",
    )
    parser.add_argument(
        "--source-dir",
        default="src/artifex",
    ...
    print(f"Found {len(analyzer.modules)} Python modules in {args.source_dir}")
    ...
    # Generate artifex dependencies
    analyzer.generate_graph(
        os.path.join(args.output_dir, "artifex_dependencies"),
        include_pattern=None,
        exclude_pattern=None,
    )
    print("Generated artifex dependencies graph")

    # Generate core dependencies
    core_path = os.path.join(args.source_dir, "generative_models/core")
    if os.path.exists(core_path):
        core_analyzer = DependencyAnalyzer(core_path)
        core_analyzer.generate_graph(
            os.path.join(args.output_dir, "core_dependencies"),
            include_pattern=None,
            exclude_pattern=None,
        )
        print("Generated core dependencies graph")

    # Generate tests dependencies
    test_path = "tests"
    if os.path.exists(test_path):
        test_analyzer = DependencyAnalyzer(test_path)
        test_analyzer.generate_graph(
            os.path.join(args.output_dir, "test_dependencies"),
            include_pattern=None,
            exclude_pattern=None,
        )
        print("Generated test dependencies graph")

    # Generate report
    report = generate_dependency_report(dependencies, circular_deps)
    with open(args.report_file, "w") as f:
        f.write(report)
    print(f"Generated dependency report at {args.report_file}")
